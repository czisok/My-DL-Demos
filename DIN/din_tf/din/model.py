...
class Model(object):
    ...
    def __init__(self, user_count, item_count, cate_count, cate_list, predict_batch_size, predict_ads_num):
        ...
        self.u = tf.compat.v1.placeholder(tf.int32, [None,])  # [B]
        self.i = tf.compat.v1.placeholder(tf.int32, [None,])  # [B]
        self.j = tf.compat.v1.placeholder(tf.int32, [None,])  # [B]
        self.y = tf.compat.v1.placeholder(tf.float32, [None,])  # [B]
        self.hist_i = tf.compat.v1.placeholder(tf.int32, [None, None])  # [B, T]
        self.sl = tf.compat.v1.placeholder(tf.int32, [None,])  # [B]
        ...
        item_b = tf.compat.v1.get_variable("item_b", [item_count],
        ...
        hist_i = tf.compat.v1.layers.dense(hist_i, hidden_units, name='hist_fcn')
        ...
        # hist_j = tf.compat.v1.layers.batch_normalization(inputs = hist_j)
        hist_j = tf.compat.v1.layers.batch_normalization(inputs=hist_j, reuse=True)
        hist_j = tf.reshape(hist_j, [-1, hidden_units], name='hist_bn')
        hist_j = tf.compat.v1.layers.dense(hist_j, hidden_units, name='hist_fcn', reuse=True)
        ...
        d_layer_1_i = tf.compat.v1.layers.dense(din_i, 80, activation=tf.nn.sigmoid, name='f1')
        ...
        d_layer_2_i = tf.compat.v1.layers.dense(d_layer_1_i, 40, activation=tf.nn.sigmoid, name='f2')
        ...
        d_layer_1_j = tf.compat.v1.layers.dense(din_j, 80, activation=tf.nn.sigmoid, name='f1', reuse=True)
        ...
        d_layer_2_j = tf.compat.v1.layers.dense(d_layer_1_j, 40, activation=tf.nn.sigmoid, name='f2', reuse=True)
        ...
        hist_sub = tf.compat.v1.layers.batch_normalization(inputs=hist_sub, name='hist_bn', reuse=tf.compat.v1.AUTO_REUSE)
        # print hist_sub.get_shape().as_list()
        hist_sub = tf.reshape(hist_sub, [-1, hidden_units])
        hist_sub = tf.compat.v1.layers.dense(hist_sub, hidden_units, name='hist_fcn', reuse=tf.compat.v1.AUTO_REUSE)
        ...
        din_sub = tf.compat.v1.layers.batch_normalization(inputs=din_sub, name='b1', reuse=True)
        d_layer_1_sub = tf.compat.v1.layers.dense(din_sub, 80, activation=tf.nn.sigmoid, name='f1', reuse=True)
        # d_layer_1_sub = dice(d_layer_1_sub, name='dice_1_sub')
        d_layer_2_sub = tf.compat.v1.layers.dense(d_layer_1_sub, 40, activation=tf.nn.sigmoid, name='f2', reuse=True)
        # d_layer_2_sub = dice(d_layer_2_sub, name='dice_2_sub')
        d_layer_3_sub = tf.compat.v1.layers.dense(d_layer_2_sub, 1, activation=None, name='f3', reuse=True)
        ...
        self.mf_auc = tf.reduce_mean(tf.cast(x > 0, tf.float32))
...
def attention(queries, keys, keys_length):
    ...
    d_layer_1_all = tf.compat.v1.layers.dense(din_all, 80, activation=tf.nn.sigmoid, name='f1_att', reuse=tf.compat.v1.AUTO_REUSE)
    d_layer_2_all = tf.compat.v1.layers.dense(d_layer_1_all, 40, activation=tf.nn.sigmoid, name='f2_att', reuse=tf.compat.v1.AUTO_REUSE)
    d_layer_3_all = tf.compat.v1.layers.dense(d_layer_2_all, 1, activation=None, name='f3_att', reuse=tf.compat.v1.AUTO_REUSE)
